...
const insertBody = (element: HTMLElement, parent : HTMLElement = document.body) => parent.insertBefore(element, parent.lastChild);

const removeBody = (element: HTMLElement, parent: HTMLElement = document.body) => parent.removeChild(element);
...
const setCordsPosition = (element: HTMLElement, parent: HTMLElement, position: "top" | "right" | "left" | "bottom") => {
  const cords = parent.getBoundingClientRect()
  const x = cords.x
  const y = cords.y
  const w = cords.width
